      ...
      const sessionsWithCounts = await Promise.all(
        (data || [] as Session[]).map(async (session: Session) => {
          ...
          try {
            ...
            console.warn("Could not get message count for session:", session.id)
          ...
          return {
            ...
            title: session.title,
            model: session.model,
            provider: session.provider || "openai", // Default provider
                      ...
                      <div className="flex-1 min-w-0">
                        {editingId === session.id ? (
                          <Input
                            ...
                            onBlur={() => {
                              if (editTitle.trim()) {
                                handleRenameSession(session.id, editTitle.trim())
                              } else {
                                setEditingId(null)
                              }
                            }}
